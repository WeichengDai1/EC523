...
def patients_to_slices(dataset, patiens_num):
    ...
    if "ACDC" in dataset:
        ref_dict = {"1": 23, "3": 68, "7": 136,
                    "14": 256, "21": 396, "28": 512, "35": 664, "140": 1216}
...
def train(args, snapshot_path):
    ...
    model = ISD(K=args.K, m=0.99, Ts=args.T_s, Tt=args.T_t, num_classes=num_classes, latent_pooling_size=args.latent_pooling_size, 
                ...
                train_encoder=args.train_encoder, train_decoder=args.train_decoder).cuda() # args.train_encoder # args.train_decoder
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            teacher_batch = copy.deepcopy(sampled_batch)
            student_batch = copy.deepcopy(sampled_batch)
            ...
            # teacher_batch, teacher_label = teacher_batch.type(torch.FloatTensor).cuda(), teacher_label.type(torch.LongTensor).cuda()
            # student_batch, student_label = student_batch.type(torch.FloatTensor).cuda(), student_label.type(torch.LongTensor).cuda()
            ...
            outputs, ema_output, uncertainty, ema_latent_logits, latent_logits, ema_output_logits, output_logits= model(student_batch, teacher_batch)
            ...
            loss = supervised_loss + consistency_weight * consistency_loss + args.k1* loss_latent + args.k2* loss_output
