...
class ISD(nn.Module):
    def __init__(self, K=48, m=0.99, Ts=0.1, Tt = 0.01, num_classes=4, train_encoder=True, train_decoder=True, 
                latent_pooling_size=1, latent_feature_size=256, output_pooling_size=16): # K=48
        ...
        self.model = create_model(num_classes=num_classes, train_encoder=train_encoder, train_decoder=train_decoder)
        # self.model_seg = _FCNHead(in_channels=last_layer_size, channels=num_classes)
        # if not train_encoder:
        #     self.model_seg.requires_grad_ = False

        self.ema_model = create_model(ema=True, num_classes=num_classes, train_encoder=False, train_decoder=False)
        # self.ema_model_seg = _FCNHead(in_channels=last_layer_size, channels=num_classes).requires_grad_(False)

        self.k_latent_head = MLP(input_channels=256, num_class=self.latent_feature_size, pooling_size=latent_pooling_size)

        self.q_latent_head = MLP(input_channels=256, num_class=self.latent_feature_size, pooling_size=latent_pooling_size)

        self.latent_predictor = nn.Sequential(
            nn.Linear(self.latent_feature_size, self.latent_feature_size),
            # nn.BatchNorm1d(self.latent_feature_size),
            # nn.ReLU(inplace=True),
            nn.Linear(self.latent_feature_size, self.latent_feature_size),
        )

        
        self.k_outputs_head = ProjectionHead(dim_in=num_classes, proj_dim=num_classes, output_pooling_size=output_pooling_size)
        self.q_outputs_head = ProjectionHead(dim_in=num_classes, proj_dim=num_classes, output_pooling_size=output_pooling_size)
        ...
        self.register_buffer('queue_mask', torch.randn(self.K, 36, num_classes*output_pooling_size**2))

        # normalize the queue
        self.queue = nn.functional.normalize(self.queue, dim=0)
        self.queue_mask = nn.functional.normalize(self.queue_mask, dim=0)
    ...
    def _momentum_update_key_encoder(self):
        ...
        # for param_q, param_k in zip(self.model_seg.parameters(), self.ema_model_seg.parameters()):
        #     param_k.data = param_k.data * self.m + param_q.data * (1. - self.m)
    ...
    def data_parallel(self):
        ...
        # self.model_seg = torch.nn.DataParallel(self.model_seg)
        # self.ema_model_seg = torch.nn.DataParallel(self.ema_model_seg)
        ...
        # self.queue = torch.nn.DataParallel(self.queue)
        # self.queue_mask = torch.nn.DataParallel(self.queue)
    ...
    def forward(self, im_q, im_k):
        ...
        with torch.no_grad():
            ...
            ema_inputs = im_k + noise
            ema_output_tmp, ema_latent_tmp, _ = self.ema_model(ema_inputs)


        ############################################## Monte Carlo Dropout #########################################
        T = 8
        _, _, w, h = im_k.shape
        volume_batch_r = im_k.repeat(2, 1, 1, 1)
        stride = volume_batch_r.shape[0] // 2
        preds = torch.zeros([stride * T, self.num_classes, w, h]).cuda()
        for i in range(T//2):
            ema_inputs = volume_batch_r + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2, 0.2)
            with torch.no_grad():
                preds[2 * stride * i:2 * stride * (i + 1)] = self.ema_model(ema_inputs)[0]
        preds = F.softmax(preds, dim=1)
        preds = preds.reshape(T, stride, self.num_classes, w, h).contiguous()
        ...
        with torch.no_grad():
            ...
            # lat = lat.view(batch_size, -1)
            # lat = nn.functional.normalize(lat, dim=1)
        ...
        ## TODO: if we get the output into a compressed network  ############################################################
        step = 32
        patch_size = 64
        output_stu_after_head = []
        output_tea_after_head = []
        # outputs (b_s, 4, 256, 256)
        for i in range(0, outputs.shape[2]-patch_size, step):
            for j in range(0, outputs.shape[3]-patch_size, step):
                output_stu_after_head.append(self.outputs_predictor(self.q_outputs_head(outputs[:, :, i:i+patch_size, j:j+patch_size])))
                output_tea_after_head.append(self.k_outputs_head(ema_output[:, :, i:i+patch_size, j:j+patch_size]))

        # print(output_stu_after_head[0].shape) # torch.Size([4, 2, 16, 16])
        ...
        output_stu_after_head = torch.cat(output_stu_after_head).reshape(batch_size, -1, 
                                        ...
                                        output_stu_after_head[0].shape[3]).contiguous() 
        # print(output_stu_after_head.shape) # torch.Size([b_s, 36, 2, 16, 16])
        ...
        output_tea_after_head = torch.cat(output_tea_after_head).reshape(batch_size, -1, 
                                        ...
                                        output_tea_after_head[0].shape[3]).contiguous() # torch.Size([b_s, 36, 2, 16, 16])
        ...
        # print(output_tea_after_head.shape) # torch.Size([1536, 36])
        # exit()
        # print(queue_mask.shape) # torch.Size([36, 48, 16384])
        queue_mask = queue_mask.reshape(-1, queue_mask.shape[0]).contiguous()
        # print(queue_mask.shape) # torch.Size([786432, 36])

        # compute the 4 logits
        ema_latent_logits = compute_logits(desired_compressed_lat_k, queue, self.Tt) 
        latent_logits = compute_logits(desired_compressed_lat_q, queue, self.Ts) 

        ema_output_logits = compute_logits(output_tea_after_head, queue_mask, self.Tt) 
        output_logits = compute_logits(output_stu_after_head, queue_mask, self.Ts) 
        ...
        return outputs_tmp, ema_output_tmp, uncertainty, ema_latent_logits, latent_logits, ema_output_logits, output_logits
...
if __name__ == '__main__':
    ...
    model = ISD(K=24, m=0.99, Ts=0.07, Tt=0.01, num_classes=4, train_encoder=True, train_decoder=True, 
                latent_pooling_size=1, latent_feature_size=512, output_pooling_size=8).cuda()
    model.data_parallel()
    input = torch.ones([6, 1, 256, 256]).cuda()
    for i in range(5):
        outputs, ema_output, uncertainty, ema_latent_logits, latent_logits, ema_output_logits, output_logits= model(input, input)
        # exit()
        print(outputs.shape)
